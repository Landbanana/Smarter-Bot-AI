...
local function playInstruments(self, character, instrumentIds)
    ...
    if  not (
            instrument and
            util.HasSimpleAccess(character, instrument)
        )
    then
        characterData.instrument = util.FindItem(character, inventory.FindAllItems(), nil, nil, util.GenerateIdPredicate(instrumentIds))
    end

    if  instrument and (
            util.ValsContain(character.HeldItems, instrument) or
...
local function generateWanderPatch(self, talentId, stopAfterBuffed)
                ...
                if characterData.timer:Update(ptable["deltaTime"]) then
                    ...
                    for crewmate in Character.GetFriendlyCrew(character) do
                        if not crewmate.CharacterHealth.GetAffliction(instrumentTalentData[talentId].afflictionId, false) and
                            Vector2.Distance(startPos, crewmate.WorldPosition) <= range
                        then
                            foundUnbuffed = true
                            break
                        end
    ...
    if instrumentTalentEnabled then
        ...
        if anyStopAfterBuffed then
            ...
            characterDataIndex = function(t, k)
                ...
                t[k].timer = Types.Timer.new(buffCheckDelay)
                return t[k]
            end
