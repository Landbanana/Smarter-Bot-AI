...
---@enum BaseOption
local BASEOPTION = {
    minimumCondition={P.defaults.MIN_CONDITION_PERCENTAGE, P.defaults.MAX_CONDITION_PERCENTAGE},
    minimumEquippedCondition={P.defaults.MIN_CONDITION_PERCENTAGE, P.defaults.MAX_CONDITION_PERCENTAGE},
    timeBetween={0, P.defaults.MAX_TIME_BETWEEN}
}
...
local function MakeOption(nameOrBase, default, optionType, description, min, max)
    if type(nameOrBase) ~= P.OPTION_TYPE.string then ---@cast nameOrBase BaseConfigOption
        min = nameOrBase.min
        max = nameOrBase.max
        nameOrBase = nameOrBase.name
    end
    
    if (min or max) and (optionType ~= P.OPTION_TYPE.float and optionType ~= P.OPTION_TYPE.int) then error("cannot assign min or max to a "..optionType, 2) end
...
local function getData()
    ---@type ConfigSection[]
    return {
        MakeSection("CanSleepWhenIdle", true, "Makes the AI treat most beds like chairs, sleeping in them when idle"),
        MakeSection("EquipArmor", true, "AI will attempt to equip armor inside their inventory every so often. This helps solve the issue of AI sometimes \"forgetting\" to put a helmet back on after using a diving mask, for example", {
            MakeOption(BASEOPTION.timeBetween, 30, P.OPTION_TYPE.int, "Increases the delay between AI attempting to equip armor. Lower=faster, but it really doesn't need to be low at all")
        }),
        ...
        MakeSection("SmarterLoadItems", true, "AI set to load these items will bring full ones to the empty tool/container first, replacing them in the slot, rather than just emptying the partially depleted ones and leaving your artifact case without a battery", {
            MakeSection("BatteryCells", true, "Apply this setting to AI loading battery cells", {
                MakeOption(BASEOPTION.minimumCondition, 90, P.OPTION_TYPE.float, "Minimum condition before AI ordered to load batteries will replace batteries")
            }),
            MakeSection("OxygenTanks", true, "Apply this setting to AI loading oxygen tanks",{
                MakeOption(BASEOPTION.minimumCondition, 90, P.OPTION_TYPE.float, "Minimum condition before AI ordered to load oxygen tanks will replace oxygen tanks")
            })
        }),
        MakeSection("ReplenishInventory", true, "AI will refill some of their empty reloadables while idling/waiting", {
            MakeSection("Idle", true, "Allow an AI that has no active order to occasionally leave their post to refill their inventory", {
                MakeOption("OnlyAtFriendlyOutposts", false, P.OPTION_TYPE.boolean, "AI will only replenish when docked at an outpost while idling")
            }),
            MakeSection("Wait", true, "Allow an AI that is set to the \"wait\" order to occasionally leave their post to refill their inventory", {
                MakeOption("OnlyAtFriendlyOutposts", true, P.OPTION_TYPE.boolean, "AI will only replenish when docked at an outpost while set to wait (THEY WILL BRIEFLY LEAVE THEIR POST)")
            }),
            MakeSection("BatteryCells", true, "Let the AI replenish their battery cells", {
                MakeOption(BASEOPTION.minimumCondition, 75, P.OPTION_TYPE.float, "Minimum condition of batteries in an idling/waiting AI's inventory (not equipped) before replacing them"),
                MakeOption(BASEOPTION.minimumEquippedCondition, 10, P.OPTION_TYPE.float, "Minimum condition of batteries equipped by an idling/waiting AI before replacing them")
            }),
            MakeSection("OxygenTanks", true, "Let the AI replenish their oxygen tanks", {
                MakeOption(BASEOPTION.minimumCondition, 95, P.OPTION_TYPE.float, "Minimum condition of oxygen tanks in an idling/waiting AI's inventory (not equipped) before replacing them"),
                MakeOption(BASEOPTION.minimumEquippedCondition, 10, P.OPTION_TYPE.float, "Minimum condition of oxygen tanks equipped by an idling/waiting AI before replacing them")
            }),
            MakeOption(BASEOPTION.timeBetween, 30, P.OPTION_TYPE.int, "Increases the delay between AI attempting to replenish their inventory. Lower=faster, but it really doesn't need to be low at all")
        })
    }
end

P.defaults = setmetatable(P.defaults, {
...
function P.Set(optionString, value)
    if type(optionString) == "table" then
        P.Get(-optionString)[optionString.stack[#optionString.stack]] = value
        return
    end

    local preOptionString, subOptionString = string.match(optionString, "(.+[^%.]+)%.([^%.]+)$")

    P.Get(preOptionString)[subOptionString] = value
end

---@param optionString string
---@return OptionType
---@overload fun(optionList:Namespace)
function P.Get(optionString)
    local config = P.data
...
function P.Save()
	...
	File.Write(configPath, json.serialize(P.data))
end

function P.Load()
    local defaults = P.defaults()

    P.data = (File.Exists(configPath) and json.parse(File.Read(configPath))) or {}
    ...
    local function LoadRecurse(option, optionDefault)
        ...
        if optionDefault.default ~= nil then --[[@cast optionDefault -ConfigSection]]
            ...
            if optionDefault.options ~= nil then
                for subOptionDefault in optionDefault.options do
    ...
    for subOptionDefault in defaults do
        LoadRecurse(P.data, subOptionDefault)
    end
    
    -- for v in defaults do
    --     for k, v in pairs(P.data[v.name]) do
    --         print(k)
    --     end
    -- end
    -- local configNew = {}
